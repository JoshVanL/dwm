...
tile(Monitor *m)
{
	...
	if (n > m->nmaster)
	  ...
	  //mw = m->ww - m->gappov*oe + m->gappiv*ie;
	  mw = m->ww - m->gappov*oe;
	...
	for (i = 0, my = ty = m->gappoh*oe, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, m->rmaster ? m->wx + m->ww - mw : m->wx + m->gappov*oe,
          m->wy + my, mw - (2*c->bw) - 2*m->gappiv*ie, h - (2*c->bw), 0);
			...
			resize(c, m->rmaster ? m->wx + m->gappov*oe : m->wx + mw + m->gappov*oe,
          m->wy + ty, m->ww - mw - (2*c->bw) - 2*m->gappov*oe, h - (2*c->bw), 0);
